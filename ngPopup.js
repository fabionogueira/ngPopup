...
(function(){
    var popupsContainer, ngPopupService,
        popupsRegistered = {},
        DEFAULT_OPTIONS = {
            offsetX: 0,
            offsetY: 0,
    ...
    angular
        .module('ngPopup', ['ngAnimate'])
        .config(['$routeProvider', function($routeProvider){
            $routeProvider.when(NG_POPUP_URL, {});
        }])
        .run(['$popup', '$location', "$route", '$timeout', '$rootScope', function($popup, $location, $route, $timeout, $rootScope){
...
//                            }                            
        ...
        .service('$popup', ['$animate',function($animate){
            ngPopupService.$animate = $animate;
...
//                }
                ...
                function apply(fn){
                    $element.injector().get('$timeout')(function(){
                        $element.scope().$apply(fn);
                    });
                }
                
                //exibe o popup
                apply(function(){
                    ngPopupService.$animate.removeClass($element, NG_HIDE_CLASS).then(function(){
        ...
        hide: function (elementOrId, options) {
            ...
            if ($element && $element[0].$$ngPopupIsVisible) {
                ...
                ngPopupService.$animate.addClass($element, NG_HIDE_CLASS).then(function(){
                    if (options.onComplete) onComplete();
                });
            }
...
/*
.classname {
  // A classe do elemento que deseja animar
  // Seu estado padrão (invisível).
}

.classname.visible-[classname]-add {
  // Qualquer trabalho de preparação que você precisa fazer antes de animação
  // Este é o local ideal para colocar o seu imóvel transição.
}

.classname.visible-[classname]-add-active{
  // classe durante a-animação (muitas vezes não é necessário)
}

.classname.visible-[classname]-remove {
  // classe Durante a-animação (muitas vezes não é necessário)
}

.classname.visible-[classname]-remove-active {
  // qualquer estilo durante a-animação (muitas vezes não é obrigatório)
}
